...
class LinearMapping:
    ...
    def grid2meter(self, grid):
        ...
        if self.h_size[1] == 0:
            ...
            y_abs = torch.where(
                h_abs > self.h_size[0],
                self.h_range[0] + (h_abs - self.h_size[0]) / self.h_size[1] * self.h_range[1],
        ...
        if self.w_size[1] == 0:
            ...
            x_abs = torch.where(
                w_abs > self.w_size[0],
                self.w_range[0] + (w_abs - self.w_size[0]) / self.w_size[1] * self.w_range[1],
    ...
    def meter2grid(self, meter, normalize=False):
        ...
        if self.w_size[1] == 0:
            ...
            w_abs = torch.where(
                ...
                self.w_size[0] + (x_abs - self.w_range[0]) / self.w_range[1] * self.w_size[1],
        ...
        if self.h_size[1] == 0:
            ...
            h_abs = torch.where(
                ...
                self.h_size[0] + (y_abs - self.h_range[0]) / self.h_range[1] * self.h_size[1],
        ...
        if self.d_size[1] == 0:
            ...
            d_abs = torch.where(
                ...
                self.d_size[0] + (z_abs - self.d_range[0]) / self.d_range[1] * self.d_size[1],
...
class GridMeterMapping:
    def __init__(
        ...
        if nonlinear_mode == "linear_upscale":
            ...
            self.mapping = NonLinearMapping(h_size[0], h_size[1], h_range[0], h_range[1], d_size[0], d_size[1], d_range)
            self.size_h = self.size_w = self.mapping.bev_size
            self.size_d = self.mapping.z_size
        elif nonlinear_mode == "linear":
            self.mapping = LinearMapping(h_size, h_range, h_half, w_size, w_range, w_half, d_size, d_range)
